...
- **Tests:** `.venv/bin/python -m pytest tests/ -q` → **208 passed** (observed 2026-09-03, ~1.3s; includes the first `streamlit.testing.v1.AppTest` UI tests in `tests/test_artists_ui.py`). Local venv runs Python 3.14.4; production image is `python:3.12-slim` (Dockerfile).
...
- **Passing:** `pytest` — 208 passed (full suite, observed this assessment).
